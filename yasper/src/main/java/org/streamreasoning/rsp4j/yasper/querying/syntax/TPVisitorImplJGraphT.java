...
public class TPVisitorImplJGraphT extends RSPQLBaseVisitor<CQ> {
    ...
    Logger log = LogManager.getLogger(TPVisitorImplJGraphT.class);
    public TPVisitorImplJGraphT() {
    ...
    public CQ visitWindowGraphPattern(RSPQLParser.WindowGraphPatternContext ctx) {
        ...
        windowsToTriples.put(windowIRI,new ArrayList<>());//TODO REMOVE
    ...
    public CQ visitTriplesSameSubjectPath(RSPQLParser.TriplesSameSubjectPathContext ctx) {
        ...
        if(!windowsToTriples.containsKey(windowIRI)){//TODO REMOVE
    ...
    private VarImplJGraphT createVar(String varName) {
        //if (varName.startsWith("?")) {
        //    varName = RDFUtils.trimFirst(varName);
        //}
        return new VarImplJGraphT(varName);
    }
    ...
    public ContinuousQuery<Graph<IRI, PredicateEdge>, Graph<IRI, PredicateEdge>, Binding, Binding> generateQuery() {
        ...
        windowsToTriples.forEach((windowStr, triplesLoc) -> {
            Graph<IRI, PredicateEdge> graph = RDFJGraphT.createGraph();
            for (TripleHolder triple : triplesLoc){
                RDFJGraphT.addTriplet(graph, triple.s, triple.p, triple.o);
            }
            windowsToGraphs.put(windowStr, graph);
        });

        Rstream<Binding, Binding> rstream = new Rstream<>();
        SimpleRSPQLQueryJGraphT<Binding> query = new SimpleRSPQLQueryJGraphT<Binding>(windowIRI, stream, time, win, windowsToGraphs, rstream, defaultGraphUri);
    ...
    public CQ visitSelectQuery(RSPQLParser.SelectQueryContext ctx) {
        for (RSPQLParser.ResultVarContext r : ctx.selectClause().resultVar()) {
            ...
            if (r.expression() != null) {
                ...
                projections.add(new VarImplJGraphT(RDFUtils.trimVar(var)));
            }
        }
    ...
    private Double parseOrRetrieveDataFromBinding(Binding b, String expression){
        if(isValue(expression)){
            ...
            return RDFUtils.parseDouble(b.value(new VarImplJGraphT(expression)).ntriplesString());
        }
    }
    ...
    private RDFTerm getBindingValue(Binding b, VarOrTerm varOrTerm){
        if(varOrTerm instanceof VarImplJGraphT){
    ...
    private VarOrTerm convertToVarOrTerm(String varOrTerm){
        if(varOrTerm.startsWith("?")){
            return new VarImplJGraphT(varOrTerm);
        }else{
